...
---
    This module can be run from the command line, i.e.
        ...
        python uvis_scan_monitor.py --trial --verbose --log --name
            --get_new_data --redownload_data --run_cr_reject --reprocess_fcr
            --run_ap_phot --show_ap_plot --proposals [PROPOSALS] --targets
...
from pyql.database.ql_database_interface import session, UVIS_flt_0, Anomalies

from toolbox import (CaptureOutput, check_subdirectory, display_args,
                     display_message, initialize_directories, MONITOR_DIR,
...
def check_source_shape(eccentricity, verbose, log):
    """
...
def remove_non_scans(prods_p_t_f, proposal_ql_roots, verbose, log):
    ...
    prods_p_t_f :  `astropy.table.table.Table`
        ...
        removed.
    """
    starting_count = len(prods_p_t_f)
        ...
        for proposal in proposals:
            if int(proposal) in [int(p) for p in args.proposals]:
                ...
                targets = sorted([os.path.basename(x)
                                  for x in glob(f'{dirs["data_dir"]}/{proposal}/*')])
                ...
                for target in targets:
                    if target in args.targets:
                        ...
                        filters = sorted([os.path.basename(x)
                                          for x in glob(f'{dirs["data_dir"]}/{proposal}/{target}/*')])
    ...
    def apply_crrej(self, args, output_dir=None, write_mask=True):
        ...
        if make_fcr:
            with CaptureOutput() as outputs:
                make_crcorr_file_scan_wfc3(self.filepath, mult=4,
    ...
    def detect_sources(self, args, data):
        ...
        if detected:
            ...
            if sources_detected == 1:
                ...
                for i in range(len(self.source_tbl)):
